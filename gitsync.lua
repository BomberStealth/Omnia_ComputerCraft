...
local function pullAll()
    ...
    for _, file in ipairs(FILES) do
        if downloadFile(file, file) then success = success + 1
...
local function uploadFile(localPath, remotePath)
    ...
    http.request({url = url, body = body, headers = headers, method = "PUT"})
    local timeout = os.startTimer(10)
    while true do
        local event, param1, param2 = os.pullEvent()
        if event == "http_success" and param1 == url then
            local code = param2.getResponseCode()
            param2.close()
...
local function pushAll()
    ...
    for _, file in ipairs(FILES) do
        if fs.exists(file) then
            if uploadFile(file, file) then success = success + 1
            else failed = failed + 1 end
        else
...
local function pushFile(filename)
    ...
    print("================================")
    if uploadFile(filename, filename) then print("Success!")
...
if cmd == "pull" then pullAll()
elseif cmd == "push" then
    if args[2] then pushFile(args[2]) else pushAll() end
